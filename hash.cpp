...
char *makeNiceStr(const unsigned char inp[], unsigned int n, char outp[])
{
  
   static unsigned char niceStr[] = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
...
int main()
{
   ...
   myMD5((const unsigned char *)inp, strlen(inp), hash);
   char outp[80];

   puts(makeNiceStr((const unsigned char *)hash, 16, outp));
   
   // hash a bunch of names
   
   strcpy(inp, "Delaware Pennsylvania California Texas...");
   puts(inp);
   puts(makeNiceStr(myMD5((const unsigned char *)inp, strlen(inp), hash), 16, outp));
   ...
   {     
	 ...
	 {	   
         ...
         fputs(makeNiceStr((const unsigned char *)rb, 10+rblen, outp), fp);
         fputs(" , ", fp);
      }
      for (k=0; k < 30; k++)
      {	   
