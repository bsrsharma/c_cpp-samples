...
// Open the report Database
  ...
  */
   ...
   rc = sqlite3_exec(pDb,
        "CREATE TABLE IF NOT EXISTS DEVICE_REPORT(ID INT NOT NULL,OP INT NOT NULL,TIME INT NOT NULL);",
         NULL, 0, &zErrMsg);
   if( rc!=SQLITE_OK ){
   ...
   /*
   For testing, run count times
   For normal use, run continuously as a background process or daemon
   ...
   */
   ...
   for(;;) {    
      if ((recvlen = recvfrom(serverfd, buf, BUFLEN, 0,
           (struct sockaddr * __restrict__)&client_addr,
           &slen)) < 0)
      {
